# 2026-09-13（十一）线上"改了却没生效"的真凶：部署漏建后端 + 演示链路修复

- **现场现象**：后台点「一键调度」仍报"运力不足（订单总需求超出可用车辆总容量）"、出不了方案；
  订单视角/司机端偶发直线与不合理走向；346 路订单被别的线路的车承运。
- **根因（部署流水线）**：`deploy-dev.yml` 的「Detect changed areas」用 compare API 判断是否需要重建后端。
  #139/#140 两次部署被取消时，「最近一次成功部署」（LAST_OK）查询在服务器侧网络抖动下返回空，
  对比基准于是退回 `github.event.before`（=#140 的合并提交）；#141 那次 push 只改了 SQL →
  **判定 backend=false → Package backend / Deploy backend 全部 skipped**，线上 jar 一直停在 #138（082256c3）。
  即：**#139 的"件数预算"修复从未真正上过线**，用户看到的是旧后端的表现。
  → 处置：用 `workflow_dispatch` 手动触发一次部署（该路径强制全量构建）把后端/前端/算法重建上线；
  并建议给检测步骤加兜底：**LAST_OK 取不到就全量构建；compare 返回文件数 ≥300 视为截断也全量构建**
  （该文件在 `.github/workflows/` 下，需带 `workflow` scope 的 token 才能推）。

- **同时修的业务问题（PR #142）**：
  - 运输段落库几何改「走廊优先」：先定车（含片区改派）再取几何，站间几何优先车辆运营线路走廊
    （并支持按站点反查同时覆盖两站的线路），里程/时长同步用走廊长度；
  - 订单视角读运输段缺几何时补算并落库（算法服务短暂不可用也不会缺线、不退化成直线）；
  - `route-between` 支持 `legId`，订单视角按该段运营线路走廊取真实走向（前端可视化已带 legId）；
  - 候选线路改为"每段只投给最贴合的那条线"（同时覆盖两站且经停最少），346 路订单回到渝A·B5204；
  - 司机端班次只返回本线路班次（否则发车发到别的线路，后续到站会被判"到站顺序不合法"）；
  - 算法判 `OVER_CAPACITY` 时，错误文案附带本批取/送件数、客运人数、候选车台数与货仓/客位合计。

验证：本地全链路 E2E（归集→一键调度→审核下发→司机端发车/到站/装车/运输段生命周期→寄货查询）**29 项全绿**；
后端 `yudao-module-transport` **333 测试通过**；算法 `pytest` **358 通过**；小程序 **10 个用例通过**；
线上（用部署服务器的算法）实测：归集 15 单 → 一键调度出方案（17 段全部高德真实道路、0 段直线）→ 审核下发 → 346 路司机端有任务与线路。

---

# 2026-09-13（十）补齐司机端登录账号（13800138001~04）

- **现象**：线上用「13800138001~04（周建国/李伟民/王守义/赵德柱）」登录司机端失败，返回"账号或密码不正确"；
  而 `13900001001~3`（重邮司机A/B/C）、`13800138006~9`（346路司机A / 沙坪坝司机A、B / 303路司机）可以正常登录。
- **根因**：司机端身份判定是"登录会员手机号 = `transport_driver.mobile`"，
  而 13800138001~04 的会员账号只在 `demo-member.sql` 里，**该脚本不在部署迁移清单** → 线上从未创建。
- **修复**：把这 4 个账号并入 `demo-real-orders.sql`（部署清单里、幂等 ON DUPLICATE），
  并加了只读校验（8 个司机账号：13800138001~04 与 13800138006~09）。

验证：本地 MySQL 连跑两次幂等 OK，8 个司机会员账号均已存在。

---

# 2026-09-13（九）演示订单池精简到 15 单（把运力问题从数据侧也堵住）

- **默认演示单池只留 15 张**：算法预检要求"本批件数 ≤ 实际选中车辆的货仓件数合计"，
  而单批最多 3 台车 × 每台 24 件 ≈ 72 件上限。演示单全量进池（20+ 单）时，仅件数就会触顶，
  直接 `OVER_CAPACITY`（"运力不足（订单总需求超出可用车辆总容量）"）。
  现在 `demo-real-orders.sql` 末尾把演示单池收敛到 **15 张核心单**（约 37 件）：
  346 主线 `TP346A~E`、跨区联运 `TPJTU1`（重邮→重庆交通大学）与 `TPCQ0004/0005`（黄桷垭↔南山）、
  重庆大学A区 `TPCQUA1~3`、真实线路样例 `TPDEMO1~4`；
  其余演示单置为「已创建」（仍在后台订单管理里可见，需要时再单独归集入池）。
  幂等，且**不影响用户自建/小程序下单的订单**。

验证：本地 MySQL 连跑 `demo-real-orders.sql` 两次幂等 OK；演示单池 = 15 单，池内总件数 46 ≤ 运力上限 72。

---

# 2026-09-13（八）修"运力不足（订单总需求超出可用车辆总容量）"——取批加件数预算

- **一键调度取批再加"件数预算"**：算法预检口径是 `pickups ≤ Σ货仓容量` 且 `deliveries ≤ Σ货仓容量`
  （`hybrid_optimizer._precheck`，超出直接返回 `OVER_CAPACITY` → 前端文案"运力不足（订单总需求超出可用车辆总容量）"）。
  一批 25 单（含配对货运单：取/送两侧都计入）很容易超过"实际选中的 3 台车"的总货仓件数，
  于是即使站点不超限也会"没有方案"。
  现在在**选中车辆之后**按它们的货仓件数合计裁剪本批：`AutoDispatchPlanner.capacityBudget(...)`（取实际选中车辆的总件数）
  + `capByTotalItems(...)`（超预算的订单留给下一批，不丢单；客运单不占货仓、不参与预算）。
  前端一键演示会继续下一轮，因此大池子会自然拆成"每批都在运力内"的多套方案。
  （每批最多 3 台车是算法契约上限，所以不是"加车"而是"分批出多套方案"。）

验证：后端 `yudao-module-transport` **333 个测试通过**（新增 `capacityBudget` / `capByTotalItems` 用例）。
