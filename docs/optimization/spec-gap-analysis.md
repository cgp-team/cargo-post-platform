...
> 📦 **存档说明（2026-09-21）**：本表为 2026-09-11 时点的方案对照。文中作为证据引用的演示数据 SQL（`demo-multi-leg.sql` 等）与演示设施已随运营化改造（PR #153）删除/下线。仅作历史存档。
...
| 新增站点动态验证（实测） | ✅ | 后台新建站点后**立即**出现在后台地图（站点数 10→11）与 `/bus/nearby`（0.07km、PROJECT_TRANSIT），无需重启；途经线路为空（不会自动加入线路）。默认 `user_access=真 / vehicle_access=假 / dispatch_enabled=假`，显式勾选后才放开（`StationCreateDefaultTest` 覆盖） |
...
| §64/§71 用户运输时间线/地图 | ✅ | 包裹追踪页"运输链地图"：**按段画真实道路轨迹（AMAP，实测 212/214 个点）**，无路网时回退站点直连并画虚线标注"估算"；当前段加粗 + 两端光圈 + 起点/终点/换乘站 marker + 图例 + 分段进度 + 换乘交接 + 方案说明 |

| §73/§74 导航路线来源与 ETA 落库 | ✅ | 多段规划时逐段调 `/api/v1/route`：`transport_leg.navigation_source` 记 `AMAP`/`ESTIMATED`、`navigation_polyline` 存紧凑路网轨迹，并用**高德真实里程/时长**覆盖估算值后按序推算 `estimated_departure/arrival`；算法不可用时保持 ESTIMATED（不伪装真实道路）。实测第1段 9.13km/26min/212 点、第2段 8.26km/21min/214 点 |
| 里程/时长口径统一 | ✅ | 方案与拓扑的总里程改为**各运输段真实路网里程之和**，总时长 = 各段时长 + 换乘停留(20min/次)。实测三段联运 TPDEMO3：9.13+8.26+9.73=**27.12km**，方案表 `total_distance=27.120`、拓扑 `27.12km`、时长 26+21+17+2×20=**104min** 三处一致 |
| 前端主题：白色 + 蓝色 | ✅ | 小程序默认主题切为「山泉蓝」（页面底色 #F5F8FC）；新增页面（消息中心/司机交接/调度中心/运输拓扑/包裹运输链地图）统一蓝色系，绿色仅保留在原有主题选项中；语义色只保留"异常=红" |
| 按钮与文字排版收口 | ✅ | view 当按钮处统一 `display:flex + align-items/justify-content:center`（水平垂直居中）；单行标题 `nowrap + ellipsis`，多行文本 `word-break` 防溢出（消息中心、司机任务卡、交接卡、运输链节点、分段/换乘列表） |
| §55~§58/§72 司机端任务+按钮状态机 | ✅ | 司机"货物交接"页新增"当前任务"卡（动态按钮 + 估算标注） |
| §65~§70 后台运输拓扑/订单详情/地图 | ✅ | 后台"运输拓扑"页新增**图形化运输链**（起点站 → 各段卡片（车辆/司机/里程/状态着色）→ 换乘交接节点 → 目的站）+ 候选方案对比 + 分段表 + 交接表 + 时间线；支持 `?orderId=` 直接打开 |
| §100 后台站点/线路/车辆管理展示可达性 | 🟡 | 站点管理页已加 来源/类型/用户可达/车辆可达/可调度 列 + 表单编辑 + 筛选；线路/车辆管理页未加 |
| §101/§134 调度中心四联动 UI | ✅ | 后台"调度中心"四联动：订单池 / **实时地图（百度地图：站点圆点+站名、车辆 marker+车牌、选中订单运输段连线、换乘站标记 + 图例）** / 运输详情（异常段可重调度 + 一键跳转图形化运输链）/ 事件时间线 |
...
### 演示走廊已按高德真实线路校准（含"换乘点必须真实"）

第一版演示走廊里的「学堂湾货运站」是**自造站点**（名字不在该片区、坐标也随手填），已删除。现在的走廊全部取自高德真实站名/线路：

| 线路 | 区段（真实站名） | 高德实测里程 |
| --- | --- | --- |
| 347路区间 | 邮电大学 → 海棠溪 | 5.58 km |
| 320路 | 海棠溪 → 四公里（枢纽）→ 五公里 | 5.14 km |
| 轨道3号线 | 四公里 → 重庆工商大学 | 2.49 km |

高德"公交路径规划"交叉验证（起点 重邮/邮电大学，终点 重庆工商大学）：

- **不乘地铁 / 最少步行**：`347路区间(邮电大学→海棠溪)` + `320路(海棠溪→五公里)`，12.388 km / 89 min / 步行 1167 m / ¥4 —— 与用户手机显示一致；
- **最快**：`347路区间(邮电大学→上新街)` + 轨道环线 + `轨道3号线(四公里→重庆工商大学)`，11.019 km / 80 min。

本系统规划结果（同一批真实站点，AMAP 路网里程）：

- `TPDEMO2`（邮电大学→五公里）：**2 段联运，1 次换乘（海棠溪）**，5.58 + 5.14 = **10.72 km / 70 min** —— 与高德"不乘地铁"方案同构同量级；
- `TPDEMO3`（邮电大学→重庆工商大学）：**3 段联运，2 次换乘（海棠溪、四公里）**，5.58 + 3.02 + 2.49 = **11.09 km / 93 min** —— 与高德"最快"方案 11.02 km 基本一致。
