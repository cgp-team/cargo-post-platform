...
public class DispatchServiceImpl implements DispatchService {
    ...
    /** 算法规模上限（与算法服务 app.py 契约一致）�?0 站点 / 25 订单 / 3 �?*/
    ...
    /**
     * 批次规划窗口(分钟)：算法要�?整批任务总耗时 �?窗口时长"�?     * 30 分钟（原来的半小时批次）对真实路网（高德时空时长 + 装卸作业）太紧，
     * 2 单以上很容易�?TIME_WINDOW_EXCEEDED；这里按 2 小时规划（可覆盖 yudao.dispatch.batch-minutes）�?     */
    ...
    public int collectOrders(DispatchCollectReqVO reqVO) {
        ...
        // 推荐：按勾选订单归集（orderIds 优先，前端勾选后�?ID 入池�?        if (reqVO.getOrderIds() != null && !reqVO.getOrderIds().isEmpty()) {
     ...
     * Phase 2：只有「待入池」订单可入池——客户提�?�?承运审核 �?READY_FOR_POOL �?归集入池�?
     * 任一非法明确报错（不悄悄跳过）�?
     */
    ...
    public Long createManualPlan(DispatchManualPlanReqVO reqVO) {
        ...
        // �?orderIds 顺序生成闭环经停：DEPART(场站) -> 各订单作业点 -> RETURN(场站)�?
        ...
        for (TransportOrderDO order : orders) {
            if (Objects.equals(order.getOrderType(), 1)) { // 客运：上�?+ 下车
            ...
            } else { // 货运/邮快件：终点为场�?�?揽收(村→场站)，否�?�?派�?场站→村)
    ...
    /**
     * 智能派单。任务终态（无可行解/失败）需要在异常抛出后仍然落库，�?ServiceException 不回滚�?
     */
    @Override
    ...
    public Long createSmartPlan(DispatchSmartPlanReqVO reqVO) {
        try {
            ...
            throw ex; // 业务异常原样抛出（前端展示具体原因，如容量越�?无可行解�?        } catch (Exception ex) {
    ...
    /** 智能派单主体（异常统一�?{@link #createSmartPlan} 兜底转成可读业务错误�?*/
    private Long doCreateSmartPlan(DispatchSmartPlanReqVO reqVO) {
        // 取订单池；为空直接报�?        List<TransportOrderDO> pooledOrders = orderMapper.selectList(new LambdaQueryWrapperX<TransportOrderDO>()
        ...
        // 一键智能调度（auto=true）：后端自动选场�?+ 自动挑候选车辆（实际车辆数由算法决定�?        StationDO depot;
        ...
        if (Boolean.TRUE.equals(reqVO.getAuto())) {
            ...
            // 片区分批：订单池可能同时有多个片区（如重庆邮电大学片�?+ 成都片区），
            // 混批会让车辆跨城跑几百公�?�?算法 infeasible；这里只�?最新那单所在片�?�?            // 其余片区留在池里，再次点击「一键调度」自动成下一套方案�?            pooledOrders = AutoDispatchPlanner.selectAutoBatch(pooledOrders, stationMap, MAX_ALGORITHM_ORDERS);
            depot = AutoDispatchPlanner.selectDepot(pooledOrders, stations);
            if (depot == null) {
                throw exception(STATION_NOT_EXISTS);
            }
            // 候选车辆：优先避开"已被在途方案占�?的车（同一台车不能同时跑两套方案；
            ...
            if (vehicles.isEmpty()) {
                // 全部车辆都在执行别的方案 �?退回不排除（保证能出方案，由调度员人工取舍�?                vehicles = AutoDispatchPlanner.selectVehicles(allVehicles, MAX_ALGORITHM_VEHICLES);
            }
            if (vehicles.isEmpty()) {
        ...
        // 先构建快照并做规模预检（只读，不占单）：无效输入快速失败，避免�?CAS 抢占后再抛错需要回滚�?
        ...
        // 规模上限预检：客运按人数拆单后可能超 25 单，超限直接报错而非等算�?413
        validateScaleLimit(algorithmReq);

        // P1-004 并发防护：CAS 抢占订单池（仅已入池可推进为已分配）。InnoDB 行锁会串行化并发智能派单�?
        // 后到的派单在 CAS 上阻塞至先到提交，随后因订单已非 POOLED 而影�?0 �?�?�?DISPATCH_POOL_EMPTY�?
        // 抢占成功后订单即�?ASSIGNED，方案落库后无需再改状态；算法失败/无解需显式回滚抢占（本方法
        ...
        int claimed = orderMapper.update(claim, new LambdaQueryWrapperX<TransportOrderDO>()
                ...
                .eq(TransportOrderDO::getStatus, TransportOrderStatusEnum.POOLED.getStatus()));
        if (claimed == 0) {
            throw exception(DISPATCH_POOL_EMPTY); // 池已被并发派单抢�?
        }
        ...
        // 多段联运：为方案内每个订单规划运输段（MultiLegPlanner 决定直达/2�?3段），并回写方案聚合字段�?        // 段规划失败不影响已生成的直达方案（多段是增强能力），逐单兜底记录日志�?        List<TransportLegDO> allLegs = new ArrayList<>();
        List<String> reasons = new ArrayList<>();
        // 订单 �?算法分配到的车辆/司机（同一辆车可拼多单；取货段�?该订单所属车�?派车�?        Map<Long, Long[]> orderVehicleMap = new HashMap<>();
        for (AlgorithmVehiclePlanDTO vehiclePlan : result.getVehiclePlans()) {
        ...
        if (!allLegs.isEmpty()) {
            ...
            // 方案总里程口径统一：各运输�?*真实路网里程**之和（Leg 已由高德路网回写；无路网时为估算值）
    ...
    /** 算法失败/无解时回�?CAS 抢占：已分配订单回到订单池，可重新派单（P1-004�?*/
    ...
    /**
     * 已被"在途方�?占用的车辆：方案状�?�?{待审�? 已下�? 执行中} 的经停明细里出现过的车�?     * 一键调度按片区连出多套方案时用它做避让，避免同一台车被两套在途方案同时占用�?     */
     ...
     * 落地条件（TODO）：
     *   1. 小程序换组织主体（个人主�?�?企业/组织）；
     *   2. 微信公众平台「功�?�?订阅消息」申请一次性订阅模板，模板标题与此�?templateTitle 对齐�?
     *   3. 后端 wx.miniapp.appid/secret 配成真实小程序（建议走环境变�?WX_MINIAPP_APPID/SECRET，secret 不入库）�?
     *
    ...
    /**
     * 方案真实道路地图数据（调度可视化用）�?     * 按「车�?+ 经停序号」给出每段（上一站→本站）的真实道路轨迹�?     * 高德不可�?失败时该段退化成两点直线并标�?{@code EUCLIDEAN}（不伪装真实道路）�?     */
    @Override
    public DispatchRoadmapRespVO getPlanRoadmap(Long id) {
        ...
        // 按车辆分组：组内 visitSequence 升序，相邻两站构成一�?        Map<Long, List<DispatchPlanItemDO>> byVehicle = new LinkedHashMap<>();
    ...
    /**
     * 两点之间的真实道路轨迹：调度可视化「按订单视角」绘制线路用�?     * 运输段落库时若高德不可用会退化成两点直线，这里按需补一次真实路网（服务端有 10 分钟缓存）�?     */
    @Override
    ...
    public DispatchValidateRespVO validate(DispatchValidateReqVO reqVO) {
        ...
        if (auto) {
            ...
            Map<Long, StationDO> stationMapForBatch = stations.stream()
                    .filter(s -> s.getId() != null)
                    .collect(Collectors.toMap(StationDO::getId, java.util.function.Function.identity(), (a, b) -> a));
            // �?createSmartPlan 同一批次口径：校验看到的订单数就是本次真正会被调度的订单�?            pooledOrders = AutoDispatchPlanner.selectAutoBatch(pooledOrders, stationMapForBatch, MAX_ALGORITHM_ORDERS);
            depot = AutoDispatchPlanner.selectDepot(pooledOrders, stations);
            if (depot == null) {
                throw exception(STATION_NOT_EXISTS);
            }
            ...
            // �?createSmartPlan 同口径：避开在途方案占用的车辆，全部在途时回退
            ...
            depot = validateDepotExists(reqVO.getDepotStationId());
            vehicles = vehicleMapper.selectBatchIds(reqVO.getVehicleIds());
            // 车辆存在性校验（口径对齐 createManualPlan �?validateVehicleExists�?            if (vehicles.size() < reqVO.getVehicleIds().size()) {
                throw exception(VEHICLE_NOT_EXISTS);
            }
        ...
        // 净载荷双维度口径：派送件 / 揽收�?分别累计（出程派送、返程揽收，货仓依次复用�?
        int deliveryItems = 0;
        int pickupItems = 0;
        ...
        for (TransportOrderDO order : pooledOrders) {
            if (Objects.equals(order.getOrderType(), 1)) { // 客运
                ...
                if (order.getPickupStationId() == null || order.getDeliveryStationId() == null) {
                    issues.add(timeSeqIssue(order, "上车站或下车站缺�?));
            ...
            } else { // 货运/邮快件：下车站为场站 �?揽收(村→场站)，否�?�?派�?场站→村)
                boolean isPickup = Objects.equals(order.getDeliveryStationId(), depot.getId());
                if (isPickup) {
        ...
        // 运力校验：总容�?vs 总需求，超出即预警�?
        // 载货按净载荷双维度口径：max(派送件, 揽收�? 与总货仓容量比较（与算�?solver 一致）�?
        // 不再把「派�?揽收」全部累计（旧口径把返程可用仓位�?0，闲置运力无法利用）�?
    ...
    public DispatchSettlementRespVO settlement(DispatchSettlementReqVO reqVO) {
        ...
        // 执行�?已完成方案（返程结算口径：方案状�?IN (2 执行�? 3 已完�?；方�?COMPLETED 流转留待后续迭代，当前终态为执行中）
    ...
    /** 算法规模上限预检�?0 站点 / 25 订单 / 3 车）：客运按人数拆单后可能超 25 单，超限直接报错避免等算�?413 */
    ...
    /** 从算法快照构�?站点Id -> {lon, lat} 坐标表（含场站），用于把度数里程换算为真实公�?*/
    ...
    private static String reasonCodeText(String reasonCode) {
        ...
        return switch (reasonCode) {
            ...
            case AlgorithmPlanRespDTO.REASON_TIMING_CONFLICT -> "客运�?下车时序冲突，无法排�?;
            case AlgorithmPlanRespDTO.REASON_PARTIAL_ONLY -> "当前订单组合只能部分完成，无法生成完整方�?;
            // 自研算法服务（ortools）扩展原因码：契约仅约定上三者，以下�?V2 优化新增的细分原�?
            case "TIME_WINDOW_EXCEEDED" -> "任务超出批次时间窗，无法在指定时段内完成";
            case "PRELOAD_INSUFFICIENT" -> "场站预装货物不足，无法完成派�?;
            case "DISTANCE_MATRIX_INCOMPLETE" -> "路网距离矩阵不完整，暂无法规�?;
    ...
    private StationDO validateDepotExists(Long depotStationId) {
        ...
        // 站点启用 �?可用于调度：手工派单同样要校�?是否开放调�?（不相信前端�?        if (!StationAccessUtil.dispatchEnabled(depot)) {
    ...
    /**
     * 班次线路公交骨架（Mandatory Passenger Service）：班次 �?线路 �?�?sequenceNo 排序的站点编号，
     * 去掉场站（场站由算法自动作为起终点）。shiftId 为空/班次不存�?线路无站点时返回 null（纯 VRP）�?
     */
    ...
    /** 构建算法规划请求快照：站�?= 场站 + 订单引用站点去重；skeleton 非空时该批车辆按公交骨架经停（联合调度） */
    private AlgorithmPlanReqDTO buildPlanRequest(StationDO depot, List<VehicleDO> vehicles,
        ...
        List<AlgorithmVehicleDTO> vehicleDTOs = vehicles.stream()
                .map(vehicle -> AlgorithmVehicleDTO.builder()
                        ...
                        // 公交骨架（Mandatory Passenger Service）：指定班次时该车辆按线路站点经�?
                        .skeleton(skeleton)
                        .build())
                .collect(Collectors.toList());
        // 子表一次加载，内存匹配（消除逐单查询�?N+1�?
     ...
     *  货运/邮快件按下车站是否为场站区分：场站→站点为派�?DELIVERY)，站点→场站为揽�?PICKUP)�?*/
    ...
    private int getItemCount(TransportOrderDO order, Map<Long, CargoOrderDO> cargoMap,
        ...
        } else if (Objects.equals(order.getOrderType(), 3)) { // 邮快�?
    ...
    /** 经停明细落库：visit_sequence �?1 递增；补填司机归属（按车辆当前有效人车绑定）�?
     *  预计到达时间�?{@link DispatchEstimationService#estimatePlan} 在明细落库后统一估算回写 */
    private void insertPlanItems(Long planId, Long vehicleId, List<AlgorithmRouteStopDTO> stops) {
        ...
        for (int i = 0; i < stops.size(); i++) {
            ...
            // 算法解释（Phase 5）：货运/揽收经停携带服务方式/服务�?绕行/乘客影响/原因�?
    ...
    private Long resolveDriverId(Long vehicleId) {
        // 防御：单测等�?Spring 上下文可能未注入 mapper，此时不派司机即�?
     ...
     * source �?null 时不约束来源（仅用于无明确来源的兼容场景）；建议始终显式传来源�?
     */
